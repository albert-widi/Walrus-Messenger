package com.valge.champchat.util;

/**
 * Created by Albert Widiatmoko on 07/12/13.
 */
public class IntentExtrasUtil {
    public final static String XTRAS_ACTIV_USER_ID = "USERIDACTIVXTRAS";
    public final static String XTRAS_ACTIV_USERNAME = "NAMEACTIVXTRAS";
    public final static String XTRAS_ACTIV_PRIVATEKEY = "PRIVATEKEYACTIVXTRAS";
    public final static String XTRAS_ACTIV_PHONENUMBER ="PHONENUMBERACTIVXTRAS";
    public final static String XTRAS_ACTIV_GCMID = "GCMIDACTIVXTRAS";
    public final static String XTRAS_ACTIV_SECRET_KEY = "SECRETKEYXTRAS";

    public static final String XTRAS_FRIEND_USER_ID = "frienduseridxtras";
    public static final String XTRAS_FRIEND_NAME = "friendnamextras";
    public static final String XTRAS_FRIEND_PHONENUMBER = "friendphonenumberxtras";
    public static final String XTRAS_FRIEND_GCMID = "friendgcmidxtras";
    public static final String XTRAS_FRIEND_PUBLICKEY = "friendpublickeyxtras";

    public static final String XTRAS_USER_ID = "useridxtras";
    public static final String XTRAS_USER_NAME = "usernamextras";
    public static final String XTRAS_USER_PRIVATE_KEY = "userprivatekeyxtras";
    public static final String XTRAS_USER_PHONE_NUMBER = "userphonenumberxtras";
    public static final String XTRAS_USER_SECRET_KEY = "usersecretkeyxtras";

    //phonenumberregistration
    public final static String XTRAS_NAME_USERNAME = "USERNAMEXTRAS";
    public final static String XTRAS_PHONENUMBER = "PHONENUMBERXTRAS";
    public final static String XTRAS_ACTION = "ACTIONXTRAS";
}
